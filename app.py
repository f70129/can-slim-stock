...
CAN SLIM 智能選股看板
=====================
William O'Neil 法則 | 台灣50・上櫃50・那斯達克100
...
US_UNIVERSE = [            # 那斯達克100指數成分股
    # 科技巨頭
    "AAPL", "MSFT", "NVDA", "AMZN", "META",
    "GOOGL","GOOG", "TSLA", "AVGO", "COST",
    # 科技／半導體
    "NFLX", "AMD",  "ASML", "CSCO", "ADBE",
    "QCOM", "INTU", "AMAT", "MU",   "LRCX",
    "KLAC", "MRVL", "PANW", "CRWD", "SNPS",
    "CDNS", "NXPI", "MCHP", "ON",   "TXN",
    "ARM",  "INTC", "GFS",  "ADSK", "AXON",
    # 軟體／雲端
    "NOW",  "ORCL", "WDAY", "DDOG", "ZS",
    "TEAM", "MDB",  "TTD",  "PLTR", "APP",
    "ROP",  "HON",
    # 生技／醫療
    "VRTX", "REGN", "GILD", "BIIB", "MRNA",
    "DXCM", "IDXX", "ISRG", "ILMN", "GEHC",
    "AZN",
    # 消費／電商
    "MELI", "SHOP", "BKNG", "ABNB", "SBUX",
    "LULU", "ORLY", "ROST", "DLTR", "MAR",
    "PYPL", "PDD",  "NTES", "JD",
    # 能源／公用
    "CEG",  "FSLR", "XEL",  "FANG",
    # 傳統產業
    "PCAR", "PAYX", "FAST", "ODFL", "CPRT",
    "VRSK", "ANSS", "CSGP", "CTSH", "KDP",
    "MNST", "MDLZ", "KHC",  "TMUS",
]
...
def score_canslim(symbol, hist, info, bench) -> dict:
    ...
    if hist is not None and len(hist) > 1:
        ...
        result["chg1m"] = (_safe(hist["Close"].iloc[-1]) / max(_safe(hist["Close"].iloc[-21]), 1) - 1
                           if len(hist) > 21 else 0.0)
        result["chg3m"] = (_safe(hist["Close"].iloc[-1]) / max(_safe(hist["Close"].iloc[-63]), 1) - 1
...
def screen_market(universe, market, top_n=TOP_N):
    ...
    def _process(sym):
        ...
        if h is None:
            return None
        return score_canslim(sym, h, fetch_info(sym), bench), h
...
def build_candle_chart(stock):
    df = stock.get("hist"); sym = stock["symbol"]; nm = stock.get("name", sym)
    ...
    fig.add_trace(go.Candlestick(x=d.index, open=d["Open"], high=d["High"],
        low=d["Low"], close=d["Close"], name=sym, showlegend=False,
        increasing=dict(line=dict(color=_GREEN, width=1), fillcolor=_GREEN),
        decreasing=dict(line=dict(color=_RED, width=1), fillcolor=_RED)), row=1, col=1)

    if "BB_Upper" in d.columns:
        fig.add_trace(go.Scatter(x=d.index, y=d["BB_Upper"], mode="lines",
            line=dict(color="rgba(120,120,220,0.35)", width=1), showlegend=False, hoverinfo="skip"), row=1, col=1)
        fig.add_trace(go.Scatter(x=d.index, y=d["BB_Lower"], mode="lines",
            line=dict(color="rgba(120,120,220,0.35)", width=1), fill="tonexty",
            fillcolor="rgba(100,100,200,0.06)", showlegend=False, hoverinfo="skip"), row=1, col=1)
    ...
    fig.add_trace(go.Bar(x=d.index, y=d["Volume"], marker_color=vcols, opacity=0.75, showlegend=False), row=2, col=1)
    ...
    if "MACD" in d.columns:
        ...
        fig.add_trace(go.Bar(x=d.index, y=d["MACD_Hist"], marker_color=hcols, opacity=0.65, showlegend=False), row=3, col=1)
    ...
    p = stock.get("price", 0); c1d = stock.get("chg1d", 0)
    sgn = "▲" if c1d >= 0 else "▼"; cc = _GREEN if c1d >= 0 else _RED
    fig.update_layout(
        title=dict(text=f"<b>{sym}</b>  {nm}  <span style='color:{cc}'>{p:,.2f}  {sgn}{abs(c1d*100):.2f}%</span>",
                   font=dict(size=12, color="#ddd"), x=0),
        height=520, paper_bgcolor=_BG, plot_bgcolor=_PLOT, font=dict(color="#888", size=10),
        legend=dict(orientation="h", yanchor="bottom", y=1.01, xanchor="right", x=1,
                    bgcolor="rgba(0,0,0,0)", font=dict(size=9)),
        xaxis_rangeslider_visible=False, margin=dict(l=40, r=14, t=42, b=6), hovermode="x unified",
    )
...
def build_radar(stock):
    ...
    fig.update_layout(
        polar=dict(bgcolor="#131722",
            ...
            angularaxis=dict(gridcolor="rgba(255,255,255,0.1)", tickfont=dict(color="#aaa", size=8))),
        paper_bgcolor=_BG, height=210, margin=dict(l=25, r=25, t=15, b=15), showlegend=False,
    )
    return fig
...
def render_market_tab(top, all_scores):
    ...
    for i, (col, stk) in enumerate(zip(cols, top)):
        with col:
            sym = stk["symbol"]; nm = stk.get("name", sym)
            total = stk.get("total", 0); sc_c = _scolor(float(total))
            st.markdown(f"""
            <div class="rank-card">
            ...
            st.plotly_chart(build_candle_chart(stk), use_container_width=True, key=f"main_{sym}_{i}")

            c1, c2 = st.columns([3, 2])
            with c1:
                st.plotly_chart(build_radar(stk), use_container_width=True, key=f"radar_{sym}_{i}")
            with c2:
                st.markdown("**分項得分**")
                for k, lbl, mx in [("C","C 當季EPS",25),("A","A 年獲利",20),("N","N 創新高",15),
                                    ("S","S 量能",15),("L","L 相對強度",15),("I","I 法人",5),("M","M 市場",5)]:
                    v = stk.get(k, 0); cc = _scolor(float(v), float(mx))
                    st.markdown(
    ...
    if len(all_scores) > TOP_N:
        with st.expander(f"📋 完整排行榜（共掃描 {len(all_scores)} 支）"):
            ...
            for r, s in enumerate(all_scores[:25], 1):
                rows.append({"名次": r, "代號": s["symbol"], "名稱": s.get("name", s["symbol"])[:14],
                             "總分": s.get("total", 0), "C": s.get("C", 0), "A": s.get("A", 0),
                             "N": s.get("N", 0), "S": s.get("S", 0), "L": s.get("L", 0),
...
def main():
    ...
    tz_tw = pytz.timezone("Asia/Taipei"); tz_ny = pytz.timezone("America/New_York")
    ...
    with st.spinner("🔍 CAN SLIM 掃描中，請稍候…"):
        prog = st.progress(0, text=f"掃描台灣50 ({len(TWSE_UNIVERSE)}支)…")
        twse_top, twse_all = screen_market(TWSE_UNIVERSE, "TWSE")

        prog.progress(34, text=f"掃描上櫃50 ({len(TPEX_UNIVERSE)}支)…")
        tpex_top, tpex_all = screen_market(TPEX_UNIVERSE, "TPEX")

        prog.progress(67, text=f"掃描那斯達克100 ({len(US_UNIVERSE)}支)…")
    ...
    all9 = ([(s, "🇹🇼 台灣50") for s in twse_top] +
            [(s, "🏪 上櫃50")  for s in tpex_top] +
            [(s, "🇺🇸 那斯達克") for s in us_top])
    ...
    if alerts:
        ...
        for stk, mkt, al in alerts:
            sym = stk["symbol"]; nm = stk.get("name", sym)
            pct = al["pct"] * 100; side_zh = "上方" if al["side"] == "above" else "下方"
    ...
    twse_cnt = len(twse_top); tpex_cnt = len(tpex_top); us_cnt = len(us_top)
    h1, h2, h3 = st.columns(3)
    with h1:
        st.markdown(
            f'<div style="background:#0d2a1a;border:1px solid #00ff88;border-radius:8px;padding:10px;text-align:center;">'
            f'<span style="font-size:1.3rem">🇹🇼</span><br>'
            f'<b style="color:#00ff88;font-size:1rem">台灣50</b><br>'
            f'<span style="color:#aaa;font-size:.85rem">前 {twse_cnt} 名（掃描{len(twse_all)}支）</span></div>',
            unsafe_allow_html=True)
    with h2:
        st.markdown(
            f'<div style="background:#0d1a2a;border:1px solid #00bfff;border-radius:8px;padding:10px;text-align:center;">'
            f'<span style="font-size:1.3rem">🏪</span><br>'
            f'<b style="color:#00bfff;font-size:1rem">上櫃50</b><br>'
            f'<span style="color:#aaa;font-size:.85rem">前 {tpex_cnt} 名（掃描{len(tpex_all)}支）</span></div>',
            unsafe_allow_html=True)
    with h3:
        st.markdown(
            f'<div style="background:#1a0d2a;border:1px solid #bf00ff;border-radius:8px;padding:10px;text-align:center;">'
            f'<span style="font-size:1.3rem">🇺🇸</span><br>'
            f'<b style="color:#bf00ff;font-size:1rem">那斯達克100</b><br>'
            f'<span style="color:#aaa;font-size:.85rem">前 {us_cnt} 名（掃描{len(us_all)}支）</span></div>',
            unsafe_allow_html=True)

    st.markdown("<div style='margin:12px 0'></div>", unsafe_allow_html=True)

    def _market_table(top_stocks, accent):
        if not top_stocks:
            st.warning("資料不足，請稍後重試。")
            return
        rows = []
        for rank, stk in enumerate(top_stocks, 1):
            rows.append({"名次": rank, "代號": stk["symbol"],
                         "名稱": stk.get("name", stk["symbol"])[:12],
                         "現價": f"{stk.get('price',0):,.2f}",
                         "日%": f"{stk.get('chg1d',0)*100:+.2f}%",
                         "月%": f"{stk.get('chg1m',0)*100:+.2f}%",
                         "C": stk.get("C",0), "A": stk.get("A",0),
                         "N": stk.get("N",0), "S": stk.get("S",0),
                         "L": stk.get("L",0), "總分": stk.get("total",0)})
        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True,
            column_config={
    ...
    with col1:
        st.markdown('<p style="color:#00ff88;font-weight:700;margin:0 0 6px 0">🇹🇼 台灣50 前3名</p>', unsafe_allow_html=True)
        _market_table(twse_top, "#00ff88")
    with col2:
        st.markdown('<p style="color:#00bfff;font-weight:700;margin:0 0 6px 0">🏪 上櫃50 前3名</p>', unsafe_allow_html=True)
        _market_table(tpex_top, "#00bfff")
    with col3:
        st.markdown('<p style="color:#bf00ff;font-weight:700;margin:0 0 6px 0">🇺🇸 那斯達克100 前3名</p>', unsafe_allow_html=True)
        _market_table(us_top, "#bf00ff")

    st.markdown("---")
    st.markdown("### 📈 各市場詳細技術分析（K線・EMA21・MACD・雷達圖）")
    tab1, tab2, tab3 = st.tabs([
        f"🇹🇼 台灣50 前{twse_cnt}名",
        f"🏪 上櫃50 前{tpex_cnt}名",
        f"🇺🇸 那斯達克100 前{us_cnt}名",
    ])
...
**21日均線警示**：收盤價距EMA21在 **±1.5%** 以內時自動顯示警示框。

> ⚠️ 免責聲明：本看板僅供學習參考，不構成投資建議。資料來源：Yahoo Finance。
        """)

    st.markdown(
        f'<div style="text-align:center;color:#333;font-size:.72rem;padding:10px 0">'
        f"CAN SLIM © William J. O'Neil ｜ 資料來源: Yahoo Finance ｜ "
